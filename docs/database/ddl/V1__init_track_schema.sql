...
CREATE DATABASE IF NOT EXISTS `s2s_track`
...
-- -----------------------------------------------------------------------------
-- 授权：把埋点库的读写权限授予业务应用账号
-- -----------------------------------------------------------------------------
-- MySQL 容器 entrypoint 只会把 MYSQL_DATABASE（业务库）的权限授予 MYSQL_USER，
-- 埋点库是本脚本自建的，不在其授权范围内。不补这条授权，应用的埋点 datasource
-- 会以 Access denied 失败 —— 且埋点是异步旁路，失败往往只体现为「看板没数据」，
-- 排查成本远高于启动即报错。
--
-- 账号名与 .env 的 MYSQL_USER 必须一致（当前口径：s2s，见 deploy/.env.example）。
-- 改 .env 中的 MYSQL_USER 必须同步改这里，否则埋点写入静默失败。
-- 只授 DML 不授 DDL：月表预建由运维任务用 root 执行，应用无需建表权限。
GRANT SELECT, INSERT, UPDATE, DELETE ON `s2s_track`.* TO 's2s'@'%';
FLUSH PRIVILEGES;
