...
> 定位：**本文是编码规范的唯一正文**。`.trae/rules/` 下的规则文件只是本文的常驻摘要/分派索引，内容一律以本文为准。
> 结构：§0–§6 为编码期常驻内容；§7–§11 为按需查阅（测试门禁／出包／编码期缺口／通用基线／变更记录）。
## 0. 总则
...
| DevSecOps | [DevSecOps接入方案.md](file:///d:/developer/code/aicoding/s2s/docs/architecture/DevSecOps接入方案.md) | 部署 | [部署架构设计文档.md](file:///d:/developer/code/aicoding/s2s/deploy/docs/部署架构设计文档.md) |

### 0.2 三条全局硬纪律（违反以缺陷论处）

| 纪律 | 落地要求 | 出处 |
| --- | --- | --- |
| **不复制字面量** | 阈值/TTL/上限/超时/批次大小只引用常量名：Dart 用 `NfrPerf`/`NfrCache`/`NfrApi` 等，Java 在 `common` 建同名常量类对应 | 详设 §0.1 |
| **不新增错误码** | 唯一口径源 PRD §12.5（24 码 + `0` = 25 枚举）；代码侧禁占号，新增只能由需求侧在 PRD 定案 | 详设 §0.1、§2.3 |
| **不凭位置信重试** | 重试/重放/重发靠显式「containsKey 才写」保全 `Idempotency-Key` 与 `X-Interaction-Id`，不依赖拦截器顺序——两种 dio 行为下只有显式判断恒对 | 详设 §11.1.1 |

### 0.3 未覆盖主题
先查依据源；依据源也没有的按 §10「通用基线」执行，**禁在业务代码中自创口径**。

## 1. 组件抽提与代码复用（反冗余硬要求）

### 1.1 总原则
1. **同一逻辑第 2 次出现前必须提取为共享实现**，禁复制-粘贴改名。范围含常量、正则、分支判断、序列化映射、键拼装、异常映射、限频/幂等/解密等横切动作。
2. 落点按复用半径分层，禁越级：跨域 → 后端 `common/`、前端 `lib/core/`；域内 → 域内私有方法/类，**不上浮** common；跨端同算法（grid_id、缓存键五要素、错误码表）→ 双端各一份，靠**共用测试向量/对照单测**保证逐位一致（跨语言镜像，双端各有唯一实现处，不算冗余）。
3. **禁防御性复制**：common 组件抄进业务域、`lib/core` 工具抄进 feature，发现即缺陷。

### 1.2 唯一实现处清单（业务代码只准调用，禁内联重写）

后端（详设 §1.2、§2、§3、§4）：

| 组件 | 唯一职责 |
| --- | --- |
| `ResponseBodyWrapper` | 唯一响应套壳与 `request_id` 注入；controller 直接返 DTO，禁手写 `ApiResponse.ok(...)` |
| `GlobalExceptionHandler` | 唯一异常→错误码映射；**唯一写 `Retry-After` 头处** |
| `ErrorCode` | 唯一错误码定义 |
| `IdempotencyInterceptor` | 唯一幂等判定（Key 校验、SETNX、首次响应缓存） |
| `RateLimiter` + `RateLimitKeys` | 唯一限频计数与键命名 |
| `CryptoFacade` | **全系统唯一解密入口**（Batch1 调用点 == 1）；`BlindIndex`/`AeadCipher` 为唯一加解密原语 |
| `GridIdCalculator` | 唯一网格算法（Java 侧） |
| `AuditLogWriter` | 唯一 `audit_log` 写入 |

前端（详设 §10–§17）：

| 组件 | 唯一职责 |
| --- | --- |
| `HeaderInterceptor` | 唯一请求头注入（三头 containsKey 才写、Authorization 每次重写） |
| `AuthRefreshInterceptor` | 唯一 40101 单飞续期 |
| `RetryInterceptor` | **全局唯一重试点**；`lib/features/` 禁 `for` 循环重试 |
| `EnvelopeInterceptor` / `unwrap` | 唯一拆信封（先拆信封再判 code） |
| `ApiErrorCode` | 唯一错误码与行为映射；各 catch 点禁自造处理 |
| `gridIdOf` | 唯一网格算法（Dart 侧） |
| `buildPinCacheKey` | 唯一 Pin 缓存键拼装 |
| `TrackReporter.buildBatch` | 唯一埋点组批（含批次换新键例外） |
| `NfrXxx` 常量类 | 唯一 NFR 阈值承载 |
...
| 代码评审 | PR 出现与 §1.2 职责重叠的代码、或复制字面量，直接打回 |
...
- 【双端】注释解释「为什么」而非「做什么」；引用设计决策处带章节号（范式见 `nfr_constants.dart`）。
...
- 【例外登记】`RateLimitThresholds`（`common/constants`）为**服务端单端正源**：客户端禁本地预测剩余次数（§5.9），Dart 端无消费方，故不建 Dart 镜像，直接抄详设 §3.4 键表（[122] KTD11，唯一允许 Java 独有 NFR 数字常量的类）。

### 3.2 错误码（索引）
错误码按前后端拆开维护，本节只做索引：
- **后端定义**（`ErrorCode` 枚举、HTTP 对齐、Retry-After 码集）→ 见 §4.13
- **前端映射**（`ApiErrorCode`、五类行为）→ 见 §5.10
- **双端契约断言** → 见 §7.2
...
- 【后端】MySQL 8.4 用 `binlog_expire_logs_seconds`（禁 `expire_logs_days`）；Redis 锁 `maxmemory 128MB` + `volatile-lru` + AOF `everysec`；会话/Token/幂等键显式 TTL，不当唯一存储（兜底是 DB 唯一约束）（技术栈 §5/§6）。
...
- `restart_window` 服务端启动自写（详设 §21 #1）。日志级别：ERROR=影响请求的未知异常；WARN=已降级/可恢复（限频命中、静默降级）；INFO=关键业务事件；DEBUG=诊断，生产默认关。
...
### 4.13 错误码定义（后端）（详设 §2.3、§12；契约）
- `ErrorCode(code, httpStatus, message, needRetryAfter)`；`code/100 == httpStatus`；`needRetryAfter=true` 共 **8 个**（`40105`、`42901`–`42907`），抛异常必须带剩余秒数，缺即实现缺陷。
- `40305` 后台 RBAC 专用（App 端不返回）；`42906` 唯一不向用户呈现；`42907` 仅未登录请求生效。
- 总数、无重复、码对齐 HTTP 由单测断言（明细见 §7.2）；前端侧的映射见 §5.10。
...
- 不采集 IMEI/MAC/IDFA/OAID；设备标识用首次启动自生成 UUID（存 shared_preferences），注释标「不可信、可被重置」。
...
### 5.10 错误码映射（前端）（详设 §12.2）
- `ApiErrorCode` = 24 服务端码 + `ok` + 本地负数码 `networkFailure(-1)` / `parseError(-2)`；行为映射 `autoRetry`/`promptWithRetryAfter`/`silentRequeue`/`forceRefetch`/`deterministicFail`/`refreshToken`（详设 §12.2 五类行为）。
- 后端侧的枚举定义见 §4.13；双端契约断言见 §7.2。
...
- **SQL 注入防护**（无项目明文口径，按通用基线）：`#{}` 预编译、禁 `${}` 接外部输入、动态列名/排序走白名单、LIKE 转义。
...
设计文档无明文口径的主题按通用惯例执行；设计文档补口径后以其为准并删本行：

> 注：**日志级别**与 **SQL 注入防护**两条已在主文件内联（分别见主文件 §4.11 与 §6），本表是同源副本——**改动须两处同改**。

| 主题 | 基线做法 | 出处状态 |
| --- | --- | --- |
| SQL 注入防护 | `#{}` 预编译；禁 `${}` 接外部输入；动态列名/排序走服务端白名单；LIKE 转义 | 无明文专条，主文件 §6 已引用 |
...
| 2026-09-19 | v1.2 | 原地压缩第一轮：条文零删减，收敛冗余措辞与重复内联引用 |
| 2026-09-19 | v1.3 | 原地压缩第二轮：§0.1 依据源改「路径根 + 仓库相对路径」（原 12 条 file:/// 全链接，省 ~470 字）；全篇去「进行/必须/全部」等虚词；删章节间 `---` 分隔线 |
| 2026-09-19 | v1.4 | 按评审意见回滚 §0.1：恢复 12 条 `file:///` 可点击链接（牺牲约 470 字换取编辑器内可跳转）；v1.3 其余压缩保留 |
| 2026-09-19 | v2.0 | **拆文件**：主文件保留 §0–§6（编码期常驻），§7–§11（测试门禁/出包/缺口/通用基线/变更记录）迁入本附录。主文件每轮注入 token 由 ~9,818 降至 ~7,600（−22%），内容零删减 |
| 2026-09-19 | v2.1 | 消除悬空交叉引用：**日志级别**条目内联进主文件 §4.11；主文件 §6 去掉「按 §10」字样、§3.2 与 §0.3 的引用改为显式指向「附录」。本附录 §10 加同源副本提示 |
| 2026-09-19 | v3.0 | **正文归一**：`.trae/rules/` 的规则文件与附录全部并回本文，本文成为唯一正文（原先规则文件是本文 v1.1 的副本，双份维护）。**§3.2 错误码按前后端拆分**：后端定义 → §4.13，前端映射 → §5.10，§3.2 降级为索引。`.trae/rules/` 改为常驻摘要 + 分派索引，不再承载正文 |
