...
# --- 加载门禁外壳（四态记账 + 汇总 + 退出码结算）------------------------------
# 外壳与判据分离：本文件只写「查什么、怎么算合格」，
# 四态语义与退出码分档由 gate-lib.sh 统一实现，L3 出包门禁复用同一份。
#
# ⚠ 这两处检查不可省，也不可降级为警告：
#   本脚本刻意不用 set -e（见下方 resolve_env 处的说明），若 source 失败而不拦，
#   后续 pass/fail/skip 全是 command not found（stderr 通常被 tee 之外的地方忽略），
#   四个计数器不存在，gate_conclude 也不存在，脚本会以最后一条命令的退出码收尾 ——
#   极可能是 0。于是「外壳库丢了」在调用方眼里就是「门禁全部通过」。
#   这与 G2「Dockerfile 不存在却报 PASS」是同一形态的假阴性，
#   区别只是它一次性放过全部 11 项，而不是放过一项。
GATE_LIB="${DEPLOY_DIR}/scripts/gate-lib.sh"
if [[ ! -f "${GATE_LIB}" ]]; then
  echo "[ERROR] 门禁外壳库缺失：${GATE_LIB}" >&2
  echo "        外壳缺失不等于门禁通过，检查中止。" >&2
  exit 1
fi
# shellcheck source=./gate-lib.sh
if ! source "${GATE_LIB}"; then
  echo "[ERROR] 门禁外壳库加载失败：${GATE_LIB}（请检查语法：bash -n ${GATE_LIB}）" >&2
  exit 1
fi
...
# --- 汇总 --------------------------------------------------------------------
# 汇总输出、SKIP 逐条列名、退出码三档结算全部由 gate-lib.sh 的 gate_conclude
# 承担，本文件不再自行拼装 —— 出包门禁（DevSecOps §4.4）用的是同一个函数，
# 两处若各写一份，改了一处漏改另一处时「SKIP 是否阻塞」会悄悄不一致，
# 而这种不一致没有检出手段：两边都能跑通、都打印绿色结论，差别只在退出码。
gate_conclude "环境：${ENV_NAME}" \
  "人工确认项则用 GATE_MANUAL_CONFIRMED=\"G5,G7\" 声明。"
