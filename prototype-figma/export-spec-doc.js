...
### 4.1 三态（${s.CARD_STATES.length} 档，其中 ${missing} 档本轮未出稿）
...
⚠️ **样本量如实标注**：状态表里标「有实处」的是从既有画面归纳的，
标「本轮新立」的**全稿零实处**、是新拟的规格。不要以为它们是量出来的。
...
2. 本文档标「本轮未出稿」「本轮新立」的项，表示**设计稿里没有对应画面** ——
   规格有效，但没有可对照的视觉参考，实现时若发现规格不适用应回头提出，而非默默改掉；
...
| 卡片选中态 / 下架态 | 规格已定，**稿内零实现** | 见 §4.1 的说明，实现侧按登记表构造 |
| 输入框 focus / error / disabled | 规格本轮新立，**稿内零实现** | 见 §7，属新拟规格而非归纳 |
